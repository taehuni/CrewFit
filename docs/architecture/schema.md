...
> 단계 3 산출물. **문서이지 마이그레이션 파일이 아님** — 적용은 구현 단계에서 Supabase SQL Editor / 마이그레이션으로.
...
## 기존 코드와의 차이 (구현 시 교체)
- `server/config/profiles.sql`: update 정책에 `with check` 없음, `auth.uid()` 미래핑, 신규 컬럼·`user_settings` 없음, 가입 트리거가 1행만 생성 → 위 ①·②·③으로 교체(ALTER + 정책 재생성 + 트리거 함수 교체)
