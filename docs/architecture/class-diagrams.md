...
이 프로젝트는 Java 클래스 계층이 아니라 React 함수 컴포넌트·JavaScript 모듈·PostgreSQL로 구성된다. 따라서 **2절은 전체 도메인 클래스 모델**, **부록 A는 실제 함수 모듈을 클래스 표기로 표현한 구현 모델**이다. 실제로 존재하지 않는 Controller·Repository 클래스를 구현된 것처럼 추가하지 않았다.
...
회원(Profile)을 중심으로 개인 운동·코칭과 크루·피드를 하나의 도식에 연결했다. 회원과 크루의 가입 관계는 CrewMember로, 운동 기록과 게시글의 연결은 Post.activity_id로 표현한다. Profile·Activity·Region은 각각 한 번만 정의하며, 크루·피드 관계를 포함한다고 해당 화면·API의 구현 완료를 뜻하지는 않는다.
...
| Crew | `crews` | 크루마다 크루장 한 명. 가입 방식은 open 또는 approval |
| CrewMember | `crew_members` | `(crew_id, user_id)` 복합 키. 가입 상태와 글쓰기 권한을 구분 |
| Post | `posts` | 작성자 필수. 크루 태그·운동 기록 첨부는 선택 |
| PostLike | `post_likes` | `(post_id, user_id)` 복합 키. 회원당 게시글에 좋아요 한 개 |
| Comment | `comments` | 게시글에 종속되는 한 단계 댓글 |
...
- **도메인 설계 설명**에는 2절의 통합 다이어그램을 사용한다. 회원·크루·운동 기록·게시글의 연결을 한 도식에서 확인할 수 있다. 데이터 사전·전체 제약은 schema.md를 참조한다.
- **구현 책임 설명**에는 부록 A를 사용한다. 도식의 모듈을 실제 JavaScript 클래스로 구현했다고 설명하지 않는다.
- **기능 동작 설명**에는 순차 다이어그램을 사용한다. ‘설계 기준’인 크루·AI를 구현 완료 흐름으로 소개하지 않는다.
- 이 문서는 기존 결정을 시각화한 것이며 새로운 상속 구조·테이블·권한 예외를 제안하지 않는다.

## 부록 A. 기록 기능의 구현 모듈 구조

2절의 엔티티 객체가 스스로 DB를 저장하는 구조는 아니다. 현재 기록 기능은 폼·페이지가 흐름을 제어하고, 함수 모듈이 검증·조회·변경을 담당한다. 다음 함수의 인자는 가독성을 위해 축약했다.
