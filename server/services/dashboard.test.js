...
import { loadWeeklyDashboard, weekRange } from './dashboard.js';
...
function database(rows, cap = 500, failAfter = Infinity) {
  ...
  return { filters, from(table) {
    ...
    const q = {
      ...
      then(resolve, reject) { return Promise.resolve(++calls > failAfter ? { error: new Error('offline') } : { data: rows.filter(row => row.id > cursor).slice(0, cap) }).then(resolve, reject); },
    };
    return q;
  }};
}
